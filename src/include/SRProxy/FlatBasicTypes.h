...
template <> struct rootcode<char> {
  const static char code = 'B';
};
template <> struct rootcode<unsigned char> {
  const static char code = 'b';
};
template <> struct rootcode<short> {
  const static char code = 'S';
};
template <> struct rootcode<short unsigned int> {
  const static char code = 's';
};
template <> struct rootcode<int> {
  const static char code = 'I';
};
template <> struct rootcode<unsigned int> {
  const static char code = 'i';
};
template <> struct rootcode<float> {
  const static char code = 'F';
};
template <> struct rootcode<double> {
  const static char code = 'D';
};
template <> struct rootcode<long> {
  const static char code = 'L';
};
template <> struct rootcode<unsigned long> {
  const static char code = 'l';
};
template <> struct rootcode<bool> {
  const static char code = 'O';
};
...
template <> struct FlatType<long long int> {
  typedef int type;
};
...
using FlatArray =
    std::conditional_t<(N <= kMaxInlineSize), FlatInlineArray<T, N>,
                       FlatOutOfLineArray<T, N>>;
...
public:
  Flat(TTree *tr, const std::string &name, const std::string &totsize,
       const IBranchPolicy *policy)
      : Flat<std::vector<char>>(tr, name, totsize, policy) {}

  void Fill(const std::string &x) {
    Flat<std::vector<char>>::Fill(std::vector<char>(
        x.c_str(),
        x.c_str() + x.size() + 1)); // deliberately include the trailing null
  }
};
